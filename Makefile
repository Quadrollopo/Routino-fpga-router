...
install-python-deps:
	.venv/bin/pip install -q -r requirements.txt --pre
else
...
score-$(ROUTER): $(foreach b,$(BENCHMARKS),$b_$(ROUTER).wirelength $b_$(ROUTER).check)
	.venv/bin/python3 ./compute-score.py $(addsuffix _$(ROUTER), $(BENCHMARKS))
...
%_rwroute_single.phys: %_unrouted.phys | $(JAVA_CLASSPATH_TXT)
	(/usr/bin/time taskset -c 0 java -cp $$(cat $(JAVA_CLASSPATH_TXT)) $(JVM_HEAP) com.xilinx.fpga24_routing_contest.PartialRouterPhysNetlist $< $@) $(call log_and_or_display,$@.log)

## NXROUTE-POC
%_nxroute-poc.phys: %_unrouted.phys xcvu3p.device | install-python-deps fpga-interchange-schema/interchange/capnp/java.capnp
	(/usr/bin/time .venv/bin/python3 networkx-proof-of-concept-router/nxroute-poc.py $< $@) $(call log_and_or_display,$@.log)

%_routino.phys: %_unrouted.phys
	(/usr/bin/time ./routino/cmake-build-release/fpga_router $< $@) $(call log_and_or_display,$@.log)
